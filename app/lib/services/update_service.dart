...
class UpdateService {
  ...
  Future<StagedUpdate> downloadAndStage(UpdateRelease release) async {
    final shaResponse = await _client.get(release.sha256Url);
    if (shaResponse.statusCode != 200) {
      throw const UpdateException('Could not download SHA256 file.');
    }
    ...
    final zipResponse = await _client.get(release.zipUrl);
    if (zipResponse.statusCode != 200) {
      throw const UpdateException('Could not download update package.');
    }
  ...
  UpdateRelease? _parseRelease(Map<String, dynamic> release) {
    ...
    final shaUrl = sha['browser_download_url'] as String?;
